...
template MastermindVariation() {
    ...
    // Compare if tries exceeded 6
    component triesLessThan = LessEqThan(4);
    triesLessThan.in[0] <== nTries;
    triesLessThan.in[1] <== 6;   
    ...
    signal tempHash;
    tempHash <== poseidon.out;
    solnHashOut <-- (tempHash == pubSolnHash) && (triesLessThan.out == 1);
}
